...
import { ThumbsUp, Edit3, Trash2, Check, X, MoreVertical, Info, User as UserIcon, CalendarDays } from 'lucide-react';
    ...
    >
      ...
      <CardContent className={cn("p-3 flex-grow", isEditing && "pb-1")}>
        {isEditing && canEditOrDelete ? (
          ...
          <p className="text-sm text-card-foreground whitespace-pre-wrap break-words min-h-[30px] py-1 pr-6"> {/* Added pr-6 for space for menu button */}
            {card.content}
          </p>
        )}
      </CardContent>
      ...
      {!isEditing && (
        <div className="px-3 pb-2 flex items-center justify-end space-x-1 text-xs text-muted-foreground">
           <ThumbsUp className={cn("h-3.5 w-3.5", hasUpvoted && "text-primary")} />
